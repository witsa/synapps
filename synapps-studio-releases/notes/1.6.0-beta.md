...
L'acteur _Ternaire_ permet de représenter une **image** et **un texte** en fonction d'un état à trois états : _Vrai_, _Faux_ et _Défaut_. L'état est défini par une propriété booléenne _Vrai_ ou _Faux_ et une propriété _En Défaut?_.

Si aucune image n'est définie, c'est la _LED_ qui est montrée : Allumée verte pour _Vrai_, éteinte verte pour _Faux_ et clignotante pour _Défaut_ mais reste personnalisable.

![alt text](assets/1.6/ternary.gif)

#### Acteur _Multi-position_

L'acteur _Multi-position_ permet de représenter une **image** et **un texte** en fonction d'un état à plusieurs positions. L'état est défini par une propriété _Position_. Chque position est définie par sa clé, un texte, une image et si elle doit clignoter. On peut en définir jusqu'à 8.
...
L'acteur _Tuyau_ permet de représenter un segment de tuyau ou une fleche de circulation. La couleur du tuyau et son orientation sont personnalisables.
...
L'acteur _Bruleur_ permet de représenter les différents états d'un bruleur de chaufferie. L'acteur est orientable vers la gauche ou la droite par un bouton dans l'inspecteur.
...
L'acteur _Chaudière_ permet de représenter les différents états d'une chaudière de chaufferie. L'acteur est orientable vers la gauche ou la droite par un bouton dans l'inspecteur.
...
L'acteur _Pompe simple_ permet de représenter les différents états d'une pompe simple de chaufferie. L'acteur est orientable dans 4 directions (0°, 90°, 180° et 270°) par un bouton dans l'inspecteur.
...
L'acteur _Pompe double_ permet de représenter les différents états d'une pompe double de chaufferie. L'acteur est orientable dans 4 directions (0°, 90°, 180° et 270°) par un bouton dans l'inspecteur.
...
L'acteur _Vanne 2 voies_ permet de représenter les différents états d'une vanne 2 voies de chaufferie. L'acteur est orientable dans 4 directions (0°, 90°, 180° et 270°) par un bouton dans l'inspecteur.
...
L'acteur _Vanne 3 voies_ permet de représenter les différents états d'une vanne 3 voies de chaufferie. L'acteur est orientable dans 4 directions (0°, 90°, 180° et 270°) par un bouton dans l'inspecteur.
...
- le nombre réel d'acteurs instanciés dans l'élément courant. C'est à dire le nombre d'acteurs qui sont réellement présents à l'exécution, en tenant compte des acteurs :
  ...
  - dans les scènes des acteurs _Écran_
  - et ceux instanciés dynamiquement ou par script.
- le nombre d'acteurs qui contiennent une iframe, comme par exemple les acteurs _Iframe_, _Agenda_ ou _Synoptique_.
...
{: .note }
> La documentation sur les moyens d'optimiser est toujours en cours. une première version est disponible dans la [section conseils de performance](../../concepts/hints.md).
...
Certains acteurs possèdent un champ _Options_ qui permet de définir une liste d'options, chacune avec un texte et une valeur. Il est maintenant possible de référencer une clé de la librairie de texte pour le champ _Texte_ de chaque option. Elle sera automatiquement traduite selon la langue de prévisualisation de la synapp.

Sont concernés les acteurs _Liste de boutons_, _Liste déroulante_, _Boutons radio_ et _Cases à cocher_.
...
{: .warning }
> ℹCette partie est en cours de développement et sera améliorée dans les prochaines versions. Elle concerne le support des futurs REDY PILOT.
...
### Problème de 1ère connexion au REDY

Dans un nouveau projet, un problème empéchait la 1ère connexion au REDY. Ce problème est corrigé.
